...
<html class="ls-theme-green">
  ...
  <div class="ls-sidebar-inner">
    ...
    <main class="ls-main ">
        ...
        <h1 class="ls-title-intro ls-ico-user">Confirmação</h1>
...
if(isset($_GET['submit']) == "Buscar"){
    ...
    $ids = $_GET["cod_cidades"];
            ...
            $consulta1="select aca_nome, a.aca_id FROM academicos a, chamada c  WHERE a.aca_id = c.aca_id and a.cur_id = $ids;";            
            ...
            $consulta8="select aca_nome, a.aca_id FROM academicos a, matricula m WHERE a.aca_id = m.aca_id and a.cur_id = $ids AND m.mat_status='S' ;";            
            $query8=mysqli_query($link, $consulta8);
            $compareceu=mysqli_num_rows($query8);

            $consultaConf="select aca_nome, a.aca_id FROM academicos a, conf_matricula c  WHERE a.aca_id = c.aca_id and a.cur_id = $ids AND c.conf_status='S' ;";            
            $queryConf=mysqli_query($link, $consultaConf);
            $confirmou=mysqli_num_rows($queryConf);
            ...
            $curso1 = mysqli_fetch_array($queri4);
            $vas = $curso1['cur_vagas'];
            
            $vagas = $compareceu - $confirmou;
            ...
            echo '<tr><td>Confirmaram Matricula: ',$confirmou,' </td></tr>';
            echo '<tr><td>Vagas Restantes: ',$vagas,' </td></tr>';
          ...
          ?>
          <br/>
          <form name="form1" method="post" action="salva-confirmacao.php">
          <?php
           

            $consulta="select a.aca_nome, a.aca_pontuacao, a.aca_classificacao,a.cur_id,a.aca_id, cf.conf_compareceu, cf.conf_status
             from conf_matricula cf
             RIGHT JOIN academicos a ON cf.aca_id = a.aca_id 
             INNER JOIN matricula m ON m.aca_id=a.aca_id
             INNER JOIN curso c ON c.cur_id = a.cur_id
             INNER JOIN usuario u ON c.cam_id=u.cam_id
             AND c.cur_id = $ids and c.cam_id = $curso 
            ...
            <td><b>Compareceu</b></td></tr>";
             while ($i <= $quantidade){
                ...
                <td> <?php echo $status === 'S' ? '<span style="color:blue">'.$linha['aca_nome'].'<span class="label label-success"> Matrículado</span></span>': '<span style="color:red">'.$linha['aca_nome'].'</span>','</td><td>'.$linha['aca_pontuacao']."</td><td>".$linha['aca_classificacao']."</td><td>".$linha['conf_compareceu']."</td>
</div></td></tr>";
                $i++;
            }
            echo "</table>"; ?>
